...
def save_results(l, l_es, p_es, i_tr, i_u, nf, runs_n, runs_cv_folds):
		...
		if len(np.unique(l_)) in [ n_clts, n_clts-1 ]:
			if n_clts == 2:
				...
				try:
					...
					precision, recall, thresholds = precision_recall_curve(l_, p_es_pos_)
					cm.auprc.append(auc(recall, precision))
				except:
					ValueError: cm.auc.append(0.0)
					cm.auprc.append(0.0)
	...
	if n_clts == 2:
		...
		fidout.write('\nauprc\t' + str(np.mean(cm.auprc)) + '\t' + str(np.std(cm.auprc)))
	else:
		fidout.write('\nauc\t[]\t[]')
		fidout.write('\nauprc\t[]\t[]')
	fidout.write('\nconfusion matrix')
	...
	for j in range(runs_n*runs_cv_folds):
		...
		if lp.learner_type == 'rf':
			...
			clf.append(GridSearchCV(\
                                ...
                                , lp.cv_grid, cv=StratifiedKFold(lp.cv_folds\
				, shuffle=True)\
