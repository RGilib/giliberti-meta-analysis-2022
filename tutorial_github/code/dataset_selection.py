...
def read_params(args):
	...
	arg( '-thr', '--threshold', type=str, choices=["abundance", "0", "0.1", "0.01", "0.001", "0.0001"],default="abundance", help="The threshold level to generate the boolean matrix\n")
...
def main(args):
	...
	for i in range(len(pf)):
		...
		if pf.index[i] == 'include':
			if pf.iloc[i,0] != 'feature_level':
				...
				feat = [s for s in feat if (pf.iloc[i,1] in s)]
	...
	if par['threshold']=="abundance":
                ...
                f.loc[:,feat] = f.loc[:,feat].replace(to_replace='nd', value='0.0')
                f.loc[:,feat] = (f.loc[:,feat].astype("float") > float(par['threshold'])).astype("float")
                f.drop(f.loc[:,feat].columns[f.loc[:,feat].max().astype('float')==f.loc[:,feat].min().astype('float')], axis=1, inplace=True)
	if par['out_f']:
		if par['tout']:
